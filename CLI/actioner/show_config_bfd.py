...
def bfd_generate_command(in_map, key_name, cmd_prefix, match_value={}, cmd_suffix=' ;') :
    cmd_str = ''
...
def bfd_generate_peer_view_cmd(rec, cmd_prefix):
    ...
    cmd_str += bfd_generate_command(rec, 'detection-multiplier', cmd_prefix + 'detect-multiplier')

    cmd_str += bfd_generate_command(rec, 'desired-minimum-echo-receive', cmd_prefix + 'echo-interval')

    match_value = { True : '' }
    cmd_str += bfd_generate_command(rec, 'echo-active', cmd_prefix + 'echo-mode', match_value=match_value)

    cmd_str += bfd_generate_command(rec, 'required-minimum-receive', cmd_prefix + 'receive-interval')

    match_value = { False : '' }
    cmd_str += bfd_generate_command(rec, 'enabled', cmd_prefix + 'shutdown', match_value=match_value)

    cmd_str += bfd_generate_command(rec, 'desired-minimum-tx-interval', cmd_prefix + 'transmit-interval')
