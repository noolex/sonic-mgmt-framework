        ...
        for f in FlowgroupCfg:
            ...
            if 'IP_TYPE' in aclCfg:
                ...
                if aclCfg['IP_TYPE'] == "IPV6ANY":
                    if 'SRC_IPV6' in aclCfg:
                        CMDS_STRING += ' src-ip ' + getIp(aclCfg['SRC_IPV6'])
                    if 'DST_IPV6' in aclCfg:
                        CMDS_STRING += ' dst-ip ' + getIp(aclCfg['DST_IPV6'])
                else:
                    if 'SRC_IP' in aclCfg:
                        CMDS_STRING += ' src-ip ' + getIp(aclCfg['SRC_IP'])
                    if 'DST_IP' in aclCfg:
                        CMDS_STRING += ' dst-ip ' + getIp(aclCfg['DST_IP'])
            ...
            if 'IN_PORTS' in aclCfg:
                ...
                for p in inPorts:
                    port = p.strip()
                    port = port[8:]
                    if port in flowGroupPerIntf:
                        flowGroupPerIntf[port].append(rulename)
                    else:
                        flowGroupPerIntf[port] = []
                        flowGroupPerIntf[port].append(rulename)
                    intfExists = True
            CMDS_STRING += "\n"
    CMDS_STRING += 'exit\n'
    for intf in flowGroupPerIntf:
        CMDS_STRING += 'interface Ethernet ' + intf + "\n"
        groups = flowGroupPerIntf[intf]
        for g in groups:
            CMDS_STRING += 'flow-group ' + str(g) + "\n"
        CMDS_STRING += 'exit\n'
