...
var (
	queryTestRouterP = Router{config: RouterConfig{}}
	queryTestRouterI = Router{config: RouterConfig{Internal: true}}
)

func testQuery(method, queryStr string, exp *translibArgs) func(*testing.T) {
	return testQuery2(&queryTestRouterP, method, queryStr, exp)
}

func testQueryI(method, queryStr string, exp *translibArgs) func(*testing.T) {
	return testQuery2(&queryTestRouterI, method, queryStr, exp)
}

func testQuery2(rr *Router, method, queryStr string, exp *translibArgs) func(*testing.T) {
	return func(t *testing.T) {
		r := httptest.NewRequest(method, "/restconf/data/querytest?"+queryStr, nil)
		r = setContextValue(r, routerConfigContextKey, rr)

		p := translibArgs{}
		err := parseQueryParams(&p, r)
...
func TestQuery_deleteEmptyEntry(t *testing.T) {
	t.Run("=true", testQueryI("DELETE", "deleteEmptyEntry=true", &translibArgs{deleteEmpty: true}))
	t.Run("=True", testQueryI("DELETE", "deleteEmptyEntry=True", &translibArgs{deleteEmpty: true}))
	t.Run("=TRUE", testQueryI("DELETE", "deleteEmptyEntry=TRUE", &translibArgs{deleteEmpty: true}))
	t.Run("=false", testQueryI("DELETE", "deleteEmptyEntry=false", &translibArgs{deleteEmpty: false}))
	t.Run("=1", testQueryI("DELETE", "deleteEmptyEntry=1", &translibArgs{deleteEmpty: false}))
	t.Run("public", testQuery("DELETE", "deleteEmptyEntry=true", nil))
	t.Run("GET", testQueryI("GET", "deleteEmptyEntry=true", nil))
	t.Run("HEAD", testQueryI("HEAD", "deleteEmptyEntry=true", nil))
	t.Run("OPTIONS", testQueryI("OPTIONS", "deleteEmptyEntry=true", nil))
	t.Run("PUT", testQueryI("PUT", "deleteEmptyEntry=true", nil))
	t.Run("POST", testQueryI("POST", "deleteEmptyEntry=true", nil))
	t.Run("PATCH", testQueryI("PATCH", "deleteEmptyEntry=true", nil))
}
